...
class jugador(pg.sprite.Sprite):
  ...
  def update(self, direction):
    ...
    if v.daño:
      self.rect.x = v.position_collision[0] # se cambia la ubicacion del jugador 
      self.rect.y = v.position_collision[1]
      v.state_life -= 1 # se resta una vida 
      v.score -= 20 # se resta porcentaje 
  ...
  def handle_event(self, event):
    ...
    if v.player_move == True:
      self.update('left')
    else:
      v.player_mover = False

    if event.type == pg.KEYDOWN:
      if event.key == pg.K_LEFT and v.player_move == False:
        self.update('left')
      if event.key == pg.K_RIGHT and v.player_move == False:
        self.update('right')
    ...
    if event.type == pg.KEYUP and v.player_move == False:
